<?php $msg='Trash'; $msgl='Trash Diocese';?>
<?php  include('../../includes/header.php') ?>
<div class="innerright">
	<div class="users_title">
		<div class="link_users">
			<ul class="<?php if($msgl=='Diocese'){echo'pad';} ?> disp">
				<li><a href="#" id='<?php if($msgl=="Trash Diocese"){echo"active";}?>'>Trash&nbsp;Diocese</a></li>
    ...
    if (isset($_GET['d'])) {
     $did=$_GET['d'];
     $update_dioceses=mysqli_query($db,"UPDATE diocese SET diocese.d_status='active' WHERE diocese.d_id='$did'");

     if ($update_dioceses) {
     	$get_updated_dioceses=mysqli_query($db,"SELECT * FROM diocese WHERE diocese.d_id='$did' ");
     	while ($update_diocese=mysqli_fetch_assoc($get_updated_dioceses)) {
     		$diocese=$update_diocese['d_name'];
     	}
     	$insert_metric=mysqli_query($db,"INSERT INTO metric VALUES(NULL,'$user_id','Active $diocese to users  on ',current_timestamp())");
     	echo "<script>alert('ACTIVE $diocese TO USERS')</script>";
     	}	
    }
	 ?>
	...
	<div class="innerright_table">
		<table>
			...
			<tbody>
                ...
                $display_dioceses=mysqli_query($db,"SELECT * FROM diocese WHERE diocese.d_status='deleted'");

                while ($row=mysqli_fetch_assoc($display_dioceses)) {
                	$num++;
