...
<div class="innerright">
    ...
    if (isset($_GET['d'])) {
     $bsid=$_GET['d'];
     $update_brothers=mysqli_query($db,"UPDATE bands SET bands.bs_status='deleted' WHERE bands.bs_id='$bsid'");
     ...
     $insert_metric=mysqli_query($db,"INSERT INTO metric VALUES(NULL,'$user_id','Deleted $brother from List of Brothers  on ',current_timestamp())");
     	echo "<script>alert('DELETE $brother FROM LIST OF BROTHERS')</script>";
   ...
   $display_brother=mysqli_query($db,"SELECT bands.*,community.*,parish.*,diocese.* FROM bands JOIN  community ON bands.bs_cid=community.c_id JOIN parish ON bands.bs_pid=parish.p_id JOIN diocese ON bands.bs_did=diocese.d_id WHERE bands.bs_status='active'");
                ...
                while ($row=mysqli_fetch_assoc($display_brother)) {
                	...
                	<?php if ($user_role=='MD' || $user_role=='IT') {?>
                	 <td><button onclick="window.location.href='edit_bands.php?d=<?=$id?>'"><img src="../../photo/EditUser.png"></button>&nbsp;<button onclick="window.location.href='?d=<?=$id?>'"><img src="../../photo/Delete.png"></button></td>
                	 <?php } ?>
